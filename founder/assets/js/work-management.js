...
function founderDepartments(){
 const d=founderData();
 const names=[...(d.departments||[]).map(x=>x.name),...(d.employees||[]).map(x=>x.department),...(d.tasks||[]).map(x=>x.department||x.dept)];
 const clean=[...new Set(names.map(x=>String(x||'').trim()).filter(Boolean))];
 return clean.length?clean:['General'];
}
function founderRoles(){
 const d=founderData();
 const names=[...(d.roles||[]).map(x=>x.name),...(d.employees||[]).map(x=>x.role),...(d.tasks||[]).map(x=>x.role)];
 const clean=[...new Set(names.map(x=>String(x||'').trim()).filter(Boolean))];
 return clean.length?clean:['Employee'];
}
...
function openWMForm({title,subtitle='Complete the details below.',saveText='Save',fields,onSave}){
 ...
 modal.querySelector('form').onsubmit=async e=>{e.preventDefault();const submit=e.currentTarget.querySelector('[type="submit"]');let fd=new FormData(e.currentTarget),obj=Object.fromEntries(fd.entries());try{if(submit){submit.disabled=true;submit.textContent='Saving...'}const result=await onSave(obj);if(result!==false)close()}catch(err){console.error('Save failed',err);alert(err?.message||'Could not save. Please check Firebase connection and try again.')}finally{if(submit){submit.disabled=false;submit.textContent=''+saveText+''}}};
 setTimeout(()=>modal.querySelector('input,select,textarea')?.focus(),20);
}
function templateForm(t={},index=-1){
 const depts=founderDepartments();
 const roles=founderRoles();
 const businesses=founderBusinesses();
 ...
 <label>Department<select name="dept">${fieldOptions(depts,t.dept||depts[0])}</select></label><label>Role<select name="role">${fieldOptions(roles,t.role||roles[0])}</select></label>
...
function generatedTaskForm(){const depts=founderDepartments(),roles=founderRoles(),businesses=founderBusinesses(),employees=activeFounderEmployees();openWMForm({title:'Generate Task',subtitle:'Create an actual task and assign it to your team.',saveText:'Generate Task',fields:`<div class="wm-form-grid"><label>Task name<input name="name" required></label><label>Business<select name="business">${fieldOptions(businesses,'All Businesses')}</select></label><label>Department<select name="dept">${fieldOptions(depts,depts[0])}</select></label><label>Role<select name="role">${fieldOptions(roles,roles[0])}</select></label><label>Assignee<select name="assignee"><option value="">Unassigned</option>${employees.map(e=>`<option value="${esc(e.name)}">${esc(e.name)}${e.department?' · '+esc(e.department):''}</option>`).join('')}</select></label><label>Priority<select name="priority">${fieldOptions(['High','Medium','Low'],'Medium')}</select></label><label>Status<select name="status">${fieldOptions(['Backlog','In Progress','Review','Completed'],'Backlog')}</select></label><label>Due date<input type="date" name="due"></label></div>`,onSave:async o=>{if(!window.SBXTaskStore?.upsert)throw new Error('Task database is not ready. Please refresh and try again.');const task={id:'GEN-'+Date.now(),name:o.name.trim(),assignee:o.assignee||'Unassigned',business:o.business,dept:o.dept,department:o.dept,role:o.role||'Employee',priority:o.priority,status:o.status,due:o.due||'',createdAt:Date.now()};const saved=await window.SBXTaskStore.upsert(task);const normalized={...task,...saved,id:saved?.id||task.id};const existingIndex=tasks.findIndex(x=>String(x.id)===String(normalized.id));if(existingIndex>=0)tasks[existingIndex]=normalized;else tasks.unshift(normalized);renderGenerated(false);return true;}})}
...
let workflows=JSON.parse(localStorage.getItem('sbx_wm_workflows')||'[]')||[];
const readyTemplates=[];
let templates=JSON.parse(localStorage.getItem('sbx_wm_templates')||'[]')||[];
let tasks=JSON.parse(localStorage.getItem('sbx_wm_tasks')||'[]')||[];
let calendarEvents=JSON.parse(localStorage.getItem('sbx_wm_calendar_events')||'[]')||[];
let calendarCursor=new Date();
let calendarView='month';
let calendarFilters=new Set(['Task','Meeting','Holiday','Festival','Leave','Birthday','Reminder','Workflow']);
const save=()=>{localStorage.setItem('sbx_wm_templates',JSON.stringify(templates));localStorage.setItem('sbx_wm_tasks',JSON.stringify(tasks));localStorage.setItem('sbx_wm_workflows',JSON.stringify(workflows));localStorage.setItem('sbx_wm_calendar_events',JSON.stringify(calendarEvents));updateBadges();window.SBXTaskStore?.saveAll?.(tasks).then(ok=>{if(ok===false)showWMToast('Task save failed. Check Firebase connection.');});};
...
function bindDashboardActions(){document.querySelectorAll('[data-nav-page]').forEach(b=>b.onclick=()=>{document.querySelector(`.wm-nav[data-page="${b.dataset.navPage}"]`)?.click()});document.querySelectorAll('[data-action="add-task"]').forEach(b=>b.onclick=generatedTaskForm);}
...
function bindTable(){document.querySelectorAll('[data-generate]').forEach(b=>b.onclick=async()=>{try{const base=templates[+b.dataset.generate]||{};const task={...base,id:'GEN-'+Date.now(),name:base.name||'Task',department:base.dept||base.department||'General',dept:base.dept||base.department||'General',assignee:'Unassigned',status:'Backlog',createdAt:Date.now()};const saved=await window.SBXTaskStore.upsert(task);tasks.unshift({...task,...saved});renderLibrary();alert('Task generated successfully');}catch(err){console.error(err);alert(err?.message||'Task could not be saved.');}});document.querySelectorAll('[data-edit-template]').forEach(b=>b.onclick=()=>addTemplate(+b.dataset.editTemplate));}
