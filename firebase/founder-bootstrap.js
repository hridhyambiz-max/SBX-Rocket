...
async function hydrate(){
 ...
 core.employees=employees.map((p,i)=>({id:p.employeeId||p.id,name:p.name||p.email,employeeId:p.employeeId||p.id,email:p.email||"",phone:p.phone||"",role:p.designation||"Employee",department:p.department||"Unassigned",business:"Sandbox Media",businessId:p.businessId||"sandbox-media",status:p.active===false?"Disabled":"Active",firebaseUid:p.id}));
