...
window.SBXEmployee = {
 async save(form, existing=null){
  ...
  const payload={employeeId,name:form.name||employeeId,email:form.email||'',phone:form.phone||'',department:form.department||'Unassigned',designation:form.role||'Employee',businessName:business,businessId,active:(form.status||'Active')!=='Disabled',updatedAt:serverTimestamp(),createdBy:user.uid};
  ...
  if(existing?.loginDocId && existing.loginDocId!==employeeId)await deleteDoc(doc(db,'employeeLogins',existing.loginDocId));
  await setDoc(doc(db,'employeeLogins',employeeId),payload,{merge:true});await hydrate();
 },
