...
async function syncFounder(key,raw){if(syncing)return;syncing=true;try{const batch=writeBatch(db);if(key===CORE||key===WM_TASKS){const parsed=safe(raw)||{};const taskList=key===WM_TASKS?(Array.isArray(parsed)?parsed:[]):(parsed.tasks||[]);taskList.forEach(t=>{const id=String(t.id||crypto.randomUUID());const assigneeName=String(t.assignee||t.assignedToName||"").trim();const assigned=t.assignedToUid||usersByName.get(assigneeName.toLowerCase())||"";const assignedEmployeeId=String(t.assignedToEmployeeId||t.employeeId||employeeIdByName.get(assigneeName.toLowerCase())||"").trim().toUpperCase();batch.set(doc(db,"tasks",id),{title:t.name||t.title||"Task",name:t.name||t.title||"Task",description:t.description||"",businessId:t.businessId||user.businessId,businessName:t.business||t.businessName||"Sandbox Media",assignedToUid:assigned,assignedToEmployeeId,assignedToName:assigneeName||"Unassigned",assignee:assigneeName||"Unassigned",department:t.department||"",priority:t.priority||"Medium",status:t.status||"To Do",progress:Number(t.progress||0),due:t.due||t.dueDate||"",checklist:t.checklist||[],completedChecks:t.completedChecks||[],comments:t.comments||[],timeSpent:Number(t.timeSpent||0),createdByUid:user.uid,updatedAt:serverTimestamp()},{merge:true});});}
...
const originalSet=localStorage.setItem.bind(localStorage);localStorage.setItem=(k,v)=>{originalSet(k,v);if(k===CORE||k===WM_TASKS||k===ATT){clearTimeout(timer);timer=setTimeout(()=>syncFounder(k,v),350)}};
