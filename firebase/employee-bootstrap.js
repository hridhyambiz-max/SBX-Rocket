...
async function readMyTasks() {
  // Read and filter client-side so tasks created by older founder builds also appear.
  const snap=await getDocs(collection(db,"tasks"));
  const employeeId=String(user.employeeId||"").trim().toUpperCase();
  const employeeName=String(user.name||"").trim().toLowerCase();
  return snap.docs.map(d=>({id:d.id,...d.data()})).filter(t=>{
    const ids=[t.assignedToEmployeeId,t.employeeId,t.assignedEmployeeId,t.assigneeEmployeeId]
      .map(v=>String(v||"").trim().toUpperCase());
    const names=[t.assignedToName,t.assignee,t.employeeName]
      .map(v=>String(v||"").trim().toLowerCase());
    return ids.includes(employeeId) || (!!employeeName && names.includes(employeeName));
  });
}
...
async function hydrateFromCloud() {
  ...
  const [tasks,leaves,attendance,notifications,payslips,rewardRequests]=await Promise.all([
    readMyTasks(),readCollection("leaveRequests"),readCollection("attendance"),readCollection("notifications"),readCollection("payslips"),readCollection("rewardRequests")
  ]);
  ...
  if(attendance.length){
    ...
    if(current){local.attendance="in";local.break=current.status==="Break";local.checkInAt=current.checkInAt?.toMillis?.()||current.checkInAt||null;local.breakStartedAt=current.breakStartedAt?.toMillis?.()||current.breakStartedAt||null;local.totalBreakMs=Number(current.totalBreakMs||0);local.attendanceDateKey=today;}
    else {local.attendance="out";local.break=false;local.checkInAt=null;local.breakStartedAt=null;local.totalBreakMs=0;local.attendanceDateKey=null;}
  } else {
    local.attendance="out";local.break=false;local.checkInAt=null;local.breakStartedAt=null;local.totalBreakMs=0;local.attendanceDateKey=null;
  }
