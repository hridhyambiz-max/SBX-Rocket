...
async function readCollection(name, field="employeeUid") {
  const snap=await getDocs(query(collection(db,name),where(field,"==",user.uid)));
...
async function hydrateFromCloud() {
  ...
  const [tasks,leaves,attendance,notifications,payslips,rewardRequests]=await Promise.all([
    readCollection("tasks","assignedToUid"),readCollection("leaveRequests"),readCollection("attendance"),readCollection("notifications","recipientUid"),readCollection("payslips"),readCollection("rewardRequests")
  ]);
  ...
  if(local.profile){local.profile={...local.profile,fullName:user.name||local.profile.fullName,email:user.email||local.profile.email,employeeId:user.employeeId||local.profile.employeeId,department:user.department||local.profile.department,role:user.designation||local.profile.role};}
